...
# patched version of pi-gen/build.sh
...
# step 1
setVariables(){
    # grab the location of this script as our reference point, this should
    # really be at the root of pi-gen
...
doSetup(){
    ...
    # for builds on arm64 machines we do not need qemu, so remove it from our
    # dependencies
    case $(uname -m) in
		aarch64)
			...
			;;
		arm*)
    sed '/qemu/d' depends > depends-cicd
    dependencies_check "${BASE_DIR}/depends-cicd"
			;;
	esac
...
main(){
    ...
    chown -R "${HOST_UID}":"${HOST_GID}" ./deploy
